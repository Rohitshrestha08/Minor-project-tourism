...
{
    $id = $_POST['delete_id'];
    $query = mysqli_query($con,"SELECT * FROM trending WHERE trending_id='$id'");
    if ($query) {
        ...
        if ($imageData) {
            ...
            if (file_exists($filePath)) {
                if (unlink($filePath)) {
                    $deleteQuery = mysqli_query($con,"DELETE FROM trending WHERE trending_id='$id'");
                    if($deleteQuery) {
                        echo "<script>alert('trending  with ID $id has been deleted.');</script>";
                        redirect('trending.php');
                        exit();
                    } else {
...
if(isset($_POST['edit'])){
    ...
    $result=mysqli_query($con,"SELECT * FROM trending WHERE trending_id='$tempid'");
...
<div id="hoteldinput" class="mt-4">          
            ...
            <div class="topicpack">
                <div id="imageContainer">
                    <img id="hotelImage" src="<?php echo $row['image'];?>" alt="trending Image">
                ...
                <hr>
                    <div class="infohotel">
                    <h5>Trending Name:</h5>
                    <input type="text" class="field" name="name" id="name" placeholder="trending Name"value="<?php echo $row['name']; ?>"  required>
                    </div>
                    <div class="infohotel">
                    <h5>Trending Price :</h5>
                    ...
                    <div class="infohotel">
                    <h5>Trending Rating :</h5>
                    <input type="text" class="field" name="rating" id="rating" placeholder="trending rating"value="<?php echo $row['rating']; ?>" required>
                    </div>
                    <div class="infohoteld" style="width:100%;">
                    <h5>Trending Description:</h5>
                    <textarea name="description" class="field" id="description" rows="5" placeholder="trending Details" required><?php echo $row['description']; ?></textarea>
                    </div>
                    <hr>
    ...
    try {
        if(isset($_FILES['newImage']) && $_FILES['newImage']['error'] === UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($newImage['tmp_name'], $imagePath)) {
                $query = "UPDATE trending SET name='$name', price='$price', location='$location', rating='$rating', description='$description', image='$imagePath' WHERE trending_id='$id'";
            } else {
                throw new Exception("Failed to upload image.");
            }
        } else {
            $query = "UPDATE trending SET name='$name', price='$price', location='$location', rating='$rating', description='$description' WHERE trending_id='$id'";
        }
        $result = mysqli_query($con, $query);

        if($result) {
            mysqli_commit($con);
            alerting('trending updated successfully.');
            redirect('trending.php');
            exit();
        } else {
            throw new Exception("Failed to update trending.");
        }
    } catch (Exception $e) {
        mysqli_rollback($con);
