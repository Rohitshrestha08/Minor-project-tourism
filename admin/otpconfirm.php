...
if (!isset($_SESSION['admine']) && !isset($_SESSION['adminotp'])) {
...
if(isset($_POST["otpcodeword"])){
    ...
    $date = date("Y-m-d");
    $email=$_SESSION['admine'];
    $otp=$_POST['otpcode'];
    $query="SELECT * FROM `admin` WHERE `email`='$email' AND `otpstatus`=0 AND `expires`='$date' AND `otp`='$otp' ";
    $result=mysqli_query($con,$query);
    if($result){
        if(mysqli_num_rows($result) ==1){
            $sql= "UPDATE admin SET `otpstatus`=1 WHERE `email`='$email' ";
            $result=mysqli_query($con,$sql);
            if($result){
                redirect('restadminpass.php');
            }
            else{
